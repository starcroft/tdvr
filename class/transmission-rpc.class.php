...
class TransmissionRPC
{
	...
	public function __construct( $server, $port, $user, $pass)
	{
		...
		$this->rpc_url = "https://$server:$port/transmission/rpc";
		if( isset($user) )
		{
			$this->username = $user;
			$this->password = $pass;
	...
	{
		$request = array("method"=>"torrent-set","arguments"=>array());
		$request['arguments']['ids'] = array( $id );
		$request['arguments']['seedRatioLimit'] = (float)$ratio;
		$request['arguments']['seedRatioMode'] = (int)"1";
		$response = $this->curl(json_encode($request));
		print_r($response['arguments']);
		return $response["status_code"] == 200;
	}
